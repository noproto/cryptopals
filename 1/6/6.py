...
for ks in range(min_keysize, max_keysize + 1):
    edit_distance = hamming_distance(encrypted_file_bytes[0:ks*4], encrypted_file_bytes[ks*4:ks*8])
...
# Result: Most likely key sizes: 29, 5, 12
...
for ks in sorted_ks_edit_distances:
    ...
    if sorted_ks_edit_distances[0] == ks:
        best_candidate = "".join([chr(c) for c in final_key])

# Use the identified key to decrypt encrypted_file_bytes
plaintext = repeating_key_xor(encrypted_file_bytes, bytes(best_candidate, "ascii"))
print(plaintext)
